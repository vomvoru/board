jQuery(($) => {
    ...
    $DOM.send.on('click', async () => {
        ...
        await $.post('../server/action/comment.php', { content, relatedPostID: id })
        location.reload();
    })

    async function run(id){
        var { error, message, data: post } = await $.getJSON('../server/action/post.php', { id })
        ...
        var { error, message, data: comments } = await $.getJSON('../server/action/comments.php', { relatedPostID: id })
